...
let perf = {
  init: (cb) => {
    ...
    let reportPerf = function () {
      ...
      /**
       * timing.fetchStart 浏览器已经准备好去使用HTTP请求抓取文档之时的 Unix毫秒时间戳。这一时刻在检查应用的缓存之前。
       * timing.navigationStart 相同的浏览环境下卸载前一个文档结束之时的 Unix毫秒时间戳。如果没有上一个文档，则它的值相当于 PerformanceTiming.fetchStart
       * 
       */
      let perfData = {
        // 网络建连
        pervPage: filterTime(timing.fetchStart, timing.navigationStart), // 上一个页面卸载时间
        ...
        // 网络接收
        send: filterTime(timing.responseStart, timing.requestStart), // 前端从发送到接收到后端第一个字节的时间
        receive: filterTime(timing.responseEnd, timing.responseStart), // 接受页面的时间
        ...
        loadEvent: filterTime(timing.loadEventEnd, timing.loadEventStart), // 为 load 事件处理程序时间
        ...
        interactive: filterTime(timing.domInteractive, timing.navigationStart), // dom可操作时间
